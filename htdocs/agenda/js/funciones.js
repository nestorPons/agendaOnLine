const SAVE = 'save', DEL = 'del', EDIT = 'edit', VIEW = 'view', INDEX = 'index.php' , 
			...
			if(!$.isEmpty(email)){			
				if(email.indexOf('@') == -1 || email.indexOf('.')==-1){
					...
					if(!$this.hasClass('input-error')){
						...
						$this.parents('form').find('button.btn-success').prop('disabled',true)
						return false
					}
				}else{
					...
					$this.parents('form').find('button.btn-success').prop('disabled',false)
					return true;
				}
			}else{
				$this.val('')
...
$(function(){
	...
	$(document)
		...
		.on('blur','.email',function(e){validar.email.funcion($(this),e)})
