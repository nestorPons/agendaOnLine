...
echo(id)
	...
	guardar: function () {
		if (servicios.validate.form()) {
			servicios.sendAjax(SAVE, (rsp, isNew) => {
				if (rsp.success) {
				...
				$("#servicios #rowServicios" + rsp.id).fadeTo("slow", 1);
			});
		}
	},
	delete: function () {
...
echo(id)
	...
	sendAjax: function (action, callback) {
		let dlg = $('#dlgServicios'), 
			id = dlg.find('#id').val(), 
			row = dlg.find("#rowServicios" + id),
			data = $("#frmEditarServicios").serializeArray();
		...
		$.ajax({
			...
			.done(function (r, s) {
				if (s == 'success')
					typeof callback == "function" && callback(r, isNew);
				else
					alert('Error: \n');
			})
