...
data.action=SAVE
data.id=idUsuario
$.ajax({type:"POST",dataType:"json",data:data,url:INDEX,}).done(function(rsp){if(data.dateBaja==undefined){if(data.id==-1){data.id=rsp.id
usuarios.rows.create(data)
notify.success('Usuario guardado con éxito!.','Nuevo usuario',!1,$('#crearCita #cliente'))}else{usuarios.rows.edit(data)
notify.success('Usuario guardado con éxito!.','Usuario editado')}}
if(dialog.isOpen=='dlgUsuarios'){dialog.close('dlgUsuarios')
...
if($('#usuarios #id').val()!=-1){if(confirm("Deseas eliminar el cliente "+id+","+nombre+"?")){data={dateBaja:Fecha.general,status:2,nombre:nombre}
usuarios.guardar(id,data,function(){$('#rowUsuarios'+id).hide('explode').remove()})}}
dialog.close('dlgUsuarios')},},dialog:function(id){usuarios.id=id
var _fnLoad=function(r){$('#dlgUsuarios').find('#id').val(id)
if(id!=-1){var $this=$("#usuarios #rowUsuarios"+id)
...
var sinColor=$this.data('color')?false:!0
...
return!1}else{return usuarios.validate.name(value)}},name:function(name){var idName=$('#dlgUsuarios #id').val();var obj=$('#usuarios #'+normalize(name));if($('#usuarios #'+normalize(name)).length==0){return!0}else{if(obj.parents('tr').attr('id')!='rowUsuarios'+idName){notify.error('El nombre esta en uso.','Error crear usuario');return!1}else{return!0}}}}}