...
this.lbl.load()
notas.init()
worker.init();cargarDatepicker()
colorearMenuDiasSemana()
Device.init()
if(Device.isCel())localStorage.setItem('menuOpen',0)
$('.tabcontrol').tabcontrol()
window.addEventListener('offline',worker.stop);window.addEventListener('online',worker.start);$('.fnToggleCharm').click(function(){menu.nav.charm.toggle($(this))})
...
main.loader.hide()};if(!$("#main").find('#'+Fecha.id).length){admin.crearDias(_pasarDia);this.inactivas.comprobar()}else _pasarDia()},activeDay:function(){$('.activa').removeClass('activa')
...
setTimeout(function(){$this.find('.icon-ok').fadeOut()},3000)}else{echo(r)}},'json').always($this.find('.icon-load').fadeOut())}},unidadTiempo:tiempoServicios=>Math.ceil(tiempoServicios/15),inactivas:{show:function(){$('#main .disabled').removeClass('disabled');$('#btnShow').find('.menulbl').html('Ocultar').end().find('i').removeClass().addClass('lnr-indent-decrease')},hide:function(){$('#main .fuera_horario').parent().addClass('disabled')
$('#btnShow').find('.menulbl').html('Mostrar').end().find('i').removeClass().addClass('lnr-line-spacing')},click:function(){let localStorageActive=localStorage.getItem("showRows")==1,fueraHorarioVisible=$('.dia.activa').find('.fuera_horario').is(':visible'),status=localStorageActive||fueraHorarioVisible?0:1;admin.inactivas.change(status)},change:function(std,save=!0){let effect='puff',duration=1500;if(std==1){this.show()}else{std=0;this.hide()}
...
this.color()},loadData(data){if(!$.isEmpty(data.obs))data.nota=data.obs;let strSer='',classNote=($.isEmpty(data.nota))?'':'show';$.each(data.servicios,function(id,serv){strSer+=admin.lbl.service(serv)})
$('#idCita_'+data.idCita).attr('lastmod',Fecha.now()).attr('idcita',data.idCita).attr('tiempo',data.tiempo_servicios).removeClassPrefix('row_').addClass('row_'+admin.unidadTiempo(data.tiempo_servicios)).find('.nombre').attr('id',data.cliente.id).find('span').text(data.cliente.nombre).end().end().find('.servicios').html(strSer).end().find('.note').find('span').text(data.obs).end().removeClass('show').addClass(classNote)},create:function(data){if($('#idCita_'+data.idCita).length)return!1;var lbl=admin.lbl,htmlSer='',idCelda=generateId.encode(data.agenda,data.fecha,data.hora),$celda=$('#'+idCelda)
if($celda.length!==0){$.each(data.servicios,function(id,serv){htmlSer+=lbl.service(serv)})
...
typeof callback=="function"&&callback()},style:function(){let w=($('#main th').is(':visible'))?$('#main th').first().width():this.width;$('.lbl').css('z-index',2).width(w-38);this.width=w;this.color()},delete:function(data,nomens=!1,callback){let idCita=(typeof data==='object')?data.idCita:data,$this=$('#idCita_'+idCita),_del=function(){$this.hide('explode',1000,function(){$this.remove()})
admin.lbl.color()
